...
if(!empty($name) && !empty($email) && !empty($organization) && !empty($position) && !empty($phone) && !empty($contact) && !empty($message))
{
    //echo "Hi";
    ...
    $subject   = "Message sent through your culinary apprenticeship contact form.";
    $message   = "Someone has requested to be contacted via the ATP Learning Promo - Culinary Apprenticeship (http://www.atplearningpromo.com/culinaryapprenticeship/) landing page:";
    ...
    $message   .= "\n\nOrganization: ";
    $message   .= strip_tags(stripslashes($_POST['organization']));
    $message   .= "\n\nPosition: ";
    $message   .= strip_tags(stripslashes($_POST['position']));
    $message   .= "\n\nEmail Address: ";
